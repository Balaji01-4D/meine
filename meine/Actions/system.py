...
class System:
    ...
    async def ram_info(self) -> Panel:
        try:
            ...
            rampanel.add_task(f"[{foreground}]USED      % ", total=100, completed=data["USED"])
            
            if cached:
                rampanel.add_task(f"[{foreground}]CACHED    % ", total=100, completed=data["CACHED"])
            if buffers:
                rampanel.add_task(f"[{foreground}]BUFFER    % ", total=100, completed=data["BUFFER"])
            ...
            ram_table.add_row("Available", SizeHelper(available), f"{available/total*100:.1f}%")
            ram_table.add_row("Free", SizeHelper(free), f"{free/total*100:.1f}%")
            
            if active:
                ram_table.add_row("Active", SizeHelper(active), f"{active/total*100:.1f}%")
            if inactive:
                ram_table.add_row("Inactive", SizeHelper(inactive), f"{inactive/total*100:.1f}%")
            if cached:
                ram_table.add_row("Cached", SizeHelper(cached), f"{cached/total*100:.1f}%")
            
            ram_table.add_row("Swap Total", SizeHelper(swap.total), "")
            ram_table.add_row("Swap Used", SizeHelper(swap.used), f"{swap.percent:.1f}%")
            ram_table.add_row("Swap Free", SizeHelper(swap.free), f"{100-swap.percent:.1f}%")
            ...
            panel_group = Group(
                ...
                Panel(ram_table, width=70, border_style=primary, title=f"[{accent}]Detailed Memory Statistics")
            )
...
*3=*^```"*4E3) ;EEEtttt:::::tZ`
 ...
 \\___)=(___/
            ...
            sys_info.append(f"[{accent}]Memory:[{foreground}] {used_mem_gb:.1f}GB / {total_mem_gb:.1f}GB ({memory.percent}%)")
            
            
            total_disk_gb = disk_usage.total / (1024 ** 3)
            used_disk_gb = disk_usage.used / (1024 ** 3)
            sys_info.append(f"[{accent}]Disk (/):[{foreground}] {used_disk_gb:.1f}GB / {total_disk_gb:.1f}GB ({disk_usage.percent}%)")
            
            
            try:
                if self.os_type == "Linux":
                    res_output = subprocess.check_output("xrandr | grep '\\*'", shell=True).decode()
            ...
            cpu_task = progress.add_task(f"[{foreground}]CPU Usage", total=100, completed=cpu_percent)
            mem_task = progress.add_task(f"[{foreground}]Memory Usage", total=100, completed=memory.percent)
            disk_task = progress.add_task(f"[{foreground}]Disk Usage", total=100, completed=disk_usage.percent)
            
            progress_panel = Panel(
                progress,
                title=f"[{accent}]System Resources",
                border_style=primary
            )
    ...
    async def Battery(self) -> Panel:
        try:
            ...
            if battery:
                ...
                battery_info.append(f"[{accent}]Status:[{foreground}] {status_icon}{status}")
                battery_info.append(f"[{accent}]Percentage:[{foreground}] {percent}%")
                battery_info.append(f"[{accent}]Time left:[{foreground}] {time_left_str}")
                        ...
                        if os.path.exists("/sys/class/power_supply/BAT0/"):
                            ...
                            try:
                                with open(f"{bat_path}manufacturer", "r") as f:
                                    ...
                                    if manufacturer:
                                        battery_info.append(f"[{accent}]Manufacturer:[{foreground}] {manufacturer}")
                            except:
                                pass
                            ...
                            try:
                                with open(f"{bat_path}model_name", "r") as f:
                                    model = f.read().strip()
                                    if model:
                                        battery_info.append(f"[{accent}]Model:[{foreground}] {model}")
                            except:
                                pass
                            ...
                            try:
                                with open(f"{bat_path}technology", "r") as f:
                                    ...
                                    if technology:
                                        battery_info.append(f"[{accent}]Technology:[{foreground}] {technology}")
                            except:
                                pass
                            ...
                            try:
                                with open(f"{bat_path}energy_full_design", "r") as f:
                                    ...
                                    battery_info.append(f"[{accent}]Design capacity:[{foreground}] {design_capacity:.2f} Wh")
                            except:
                                pass
                            ...
                            try:
                                with open(f"{bat_path}energy_full", "r") as f:
                                    ...
                                    with open(f"{bat_path}energy_now", "r") as f2:
                                        ...
                                        battery_info.append(f"[{accent}]Current capacity:[{foreground}] {current_energy:.2f}/{full_capacity:.2f} Wh")
                                        
                                        
                                        if 'Design capacity' in '\n'.join(battery_info):
                                            health = (full_capacity / design_capacity) * 100
                                            battery_info.append(f"[{accent}]Health:[{foreground}] {health:.1f}%")
                            except:
                                pass
                            ...
                            try:
                                with open(f"{bat_path}cycle_count", "r") as f:
                                    cycles = f.read().strip()
                                    battery_info.append(f"[{accent}]Charge cycles:[{foreground}] {cycles}")
                            except:
                                pass
                    except:
                        pass
                ...
                if self.os_type == "Linux":
                    ...
                    try:
                        with open("/sys/class/power_supply/BAT0/manufacturer", "r") as f:
                            manufacturer = f.read().strip()
                            if manufacturer:
                                lines.append(f"[{accent}]Manufacturer: [{foreground}]{manufacturer}")
                    except:
                        pass
                    ...
                    try:
                        with open("/sys/class/power_supply/BAT0/technology", "r") as f:
                            technology = f.read().strip()
                            if technology:
                                lines.append(f"[{accent}]Technology: [{foreground}]{technology}")
                    except:
                        pass
                    ...
                    try:
                        with open("/sys/class/power_supply/BAT0/energy_full_design", "r") as f:
                            design_capacity = int(f.read().strip()) / 1000000  
                            lines.append(f"[{accent}]Design capacity: [{foreground}]{design_capacity:.2f} Wh")
                    except:
                        pass
                    ...
                    try:
                        with open("/sys/class/power_supply/BAT0/energy_full", "r") as f:
                            full_capacity = int(f.read().strip()) / 1000000  
                            with open("/sys/class/power_supply/BAT0/energy_now", "r") as f2:
                                current_energy = int(f2.read().strip()) / 1000000  
                                lines.append(f"[{accent}]Current capacity: [{foreground}]{current_energy:.2f}/{full_capacity:.2f} Wh")
                                
                                
                                try:
                                    with open("/sys/class/power_supply/BAT0/energy_full_design", "r") as f3:
                                        design_capacity = int(f3.read().strip()) / 1000000
                                        health = (full_capacity / design_capacity) * 100
                                        lines.append(f"[{accent}]Health: [{foreground}]{health:.1f}%")
                                except:
                                    pass
                    except:
                        pass
                    ...
                    try:
                        with open("/sys/class/power_supply/BAT0/cycle_count", "r") as f:
                            cycles = f.read().strip()
                            lines.append(f"[{accent}]Charge cycles: [{foreground}]{cycles}")
                    except:
                        pass
                ...
                progress = Progress(
                    "{task.description}",
                    BarColumn(bar_width=40, complete_style=self._get_battery_color(percent)),
                    "{task.percentage:>3.0f}%",
                    expand=True
                )
                
                battery_task = progress.add_task(f"[{foreground}]Battery Level", total=100, completed=percent)
                
                progress_panel = Panel(
                    progress,
                    title=f"[{accent}]Power Status",
                    border_style=primary
                )
            ...
            return Panel(f"[{error}]No battery information available.", border_style=primary)
        except Exception as e:
...
|___________|"""
    ...
    async def ENV(self) -> Table:
        try:
            ...
            return Table(title=f"Error getting environment variables: {e}", border_style="red")
    ...
    async def CPU(self) -> Panel:
        try:
            ...
            cpu_info_table.add_column("CPU Property", style=foreground, header_style=accent)
            cpu_info_table.add_column("Value", style=foreground, header_style=accent)
            
            
            cpu_info_table.add_row("Physical Cores", str(cpu_count_physical) if cpu_count_physical else "N/A")
            cpu_info_table.add_row("Logical Cores", str(cpu_count_logical) if cpu_count_logical else "N/A")
            
            if cpu_freq:
                if hasattr(cpu_freq, 'current') and cpu_freq.current:
                    cpu_info_table.add_row("Current Frequency", f"{cpu_freq.current:.2f} MHz")
            ...
            if len(results) > 4 and results[4]:
                ...
                if hasattr(cpu_stats, 'ctx_switches'):
                    cpu_info_table.add_row("Context Switches", f"{cpu_stats.ctx_switches:,}")
            ...
            if len(results) > 5 and results[5]:
                ...
                cpu_time_table.add_column("CPU Time", style=foreground, header_style=accent)
                cpu_time_table.add_column("Percentage", style=foreground, header_style=accent)
    ...
    async def USER(self) -> Panel:
        try:
                ...
                if self.os_type == "Linux":
                    ...
                    for user in users:
                        if user.name == username:
                            ...
                            user_table.add_row("Host", user.host if hasattr(user, 'host') else "localhost")
            ...
            return Panel(f"Error getting user information: {e}", border_style=self.safe_style('error'))
    ...
    async def DiskInfo(self):
        try:
            ...
            for header in headers:
                partition_table.add_column(header, style=foreground, header_style=accent)
            ...
            io_headers = ["Device", "Read Count", "Write Count", "Read Bytes", "Write Bytes", "Read Time", "Write Time"]
            ...
            return Panel(f"Error getting disk information: {e}", title="Disk Info Error", border_style=self.safe_style('error'))
    ...
    async def Processes(self):
        try:
            ...
            for header in headers:
                tableofproccess.add_column(header, style=foreground, header_style=accent)
            ...
            return Table(title=f"Error getting process information: {e}", border_style="red")
